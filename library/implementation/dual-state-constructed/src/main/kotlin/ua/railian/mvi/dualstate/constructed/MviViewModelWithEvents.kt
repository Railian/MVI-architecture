package ua.railian.mvi.dualstate.constructed
...
import ua.railian.mvi.constructed.MviEventEmitter
import ua.railian.mvi.constructed.MviIntentProcessor
import ua.railian.mvi.constructed.MviStateMapper
import ua.railian.mvi.constructed.MviStateReducer
...
public abstract class MviViewModelWithEvents<DOMAIN_STATE, UI_STATE, INTENT, RESULT, EVENT>(
    ...
    final override val state: StateFlow<UI_STATE> by lazy {
        core.prepareState(
            stateFlow = mviState.map(stateMapper::mapState),
            initialState = stateMapper.mapState(initialState),
        )
    }
    ...
    private suspend fun MviPipelineLogger.process(intent: INTENT) {
        ...
        intentProcessor.processIntent(intent, state).collect { result ->
            ...
            eventEmitter.emitEvent(result, state.value)
                ?.let { event -> events.send(event) }
        }
    }
    //endregion

    protected abstract val intentProcessor: MviIntentProcessor<INTENT, DOMAIN_STATE, RESULT>
    protected abstract val stateReducer: MviStateReducer<RESULT, DOMAIN_STATE>
    protected abstract val eventEmitter: MviEventEmitter<RESULT, DOMAIN_STATE, EVENT>
    protected abstract val stateMapper: MviStateMapper<DOMAIN_STATE, UI_STATE>
